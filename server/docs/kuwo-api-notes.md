...
- 本机（Windows，Git Bash）经代理/VPN 走**海外出口**，部署目标 NAS 所在内网为大陆出口。
- **区域限制是本次调研最重要的发现**：
...
（目标部署位置为内网 NAS / Docker，满足条件）。开发期若在海外出口调试直链会得到 407，属预期。
...
- 本机发现：局域网内有 HTTP 代理（海外出口）。若环境变量带 `http_proxy`，
  curl 会走代理导致 407；**加 `--noproxy '*'` 直连（大陆出口）即返回 200**——已在本地正向验证
...
6. 本机环境有系统代理（局域网 HTTP 代理，海外出口），测试大陆服务统一加 `--noproxy '*'`。
