...
│  ├─ task.ts            #   下载任务管理（8 端点）
│  ├─ config.ts          #   探活（/api/healthcheck）与音源插件元信息
...
├─ lib/                  # 纯函数工具：adapter（字段适配）、format、sanitize、runtimeConfig…
...
- **字段适配**：搜索记录与详情记录字段不一致（`musicName` vs `name` 等），统一经 `lib/adapter.ts` 适配为 `SongRecord`（如 `albumSongToRecord`、`fnosTrackToRecord`），下游组件只认一种形状。
