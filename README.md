...
- **歌曲搜索**：音源插件切换（以后端启用列表为准）、搜索联想词、分页、音质标签展示（按码率从高到低排序）
...
# 方式二：docker run
docker build -t musiccopilot .
docker run -d -p 17716:80 \
  -e MC_API_BASE_URL=http://192.168.31.170:8096 \
  -e MC_USERNAME=admin -e MC_PASSWORD=admin \
  musiccopilot
```

访问 http://localhost:17716。`MC_API_BASE_URL` 必填（nginx 反代目标）；密码避免包含 `"` 或 `\`。对外端口默认 17716，compose 部署时可在 `.env` 里用 `MC_PORT` 覆盖。容器启动失败先看 `docker logs musiccopilot`，多为缺少 `MC_API_BASE_URL`。
...
- **播放队列**：整张/整组连播、上一首/下一首、自动切歌、队列位置显示
